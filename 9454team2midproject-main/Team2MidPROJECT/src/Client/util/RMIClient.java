...
public class RMIClient {
    ...
    // shared and server both need to know how to register/unregister callbacks, so we put it here
    public static void registerCallback(String username, ClientCallback callback) {
        try {
            Registry registry = LocateRegistry.getRegistry(SERVER_HOST, RMI_PORT);
            RemoteService service = (RemoteService) registry.lookup(SERVICE_NAME);
            service.registerCallback(username, callback);
        } catch (Exception e) {
            System.err.println("[RMIClient] registerCallback failed: " + e.getMessage());
        }
    }

    // from productUpdateCallback, when client disconnects or logs out, we should unregister the callback to avoid memory leaks on server
    public static void unregisterCallback(String username) {
        try {
            Registry registry = LocateRegistry.getRegistry(SERVER_HOST, RMI_PORT);
            RemoteService service = (RemoteService) registry.lookup(SERVICE_NAME);
            service.unregisterCallback(username);
        } catch (Exception e) {
            System.err.println("[RMIClient] unregisterCallback failed: " + e.getMessage());
        }
    }
